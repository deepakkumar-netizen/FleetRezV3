...
// MODAL OPEN ADD CLASS ON BODY
const modal = document.getElementById("ModifySearchModal");

modal.addEventListener("shown.bs.modal", function () {
  document.body.classList.add("modify-modal-open");
});

modal.addEventListener("hidden.bs.modal", function () {
  document.body.classList.remove("modify-modal-open");
});

// ADD CLASS ON MAIN CARD
document.querySelectorAll(".result-card-main .collapse").forEach((collapse) => {
  collapse.addEventListener("show.bs.collapse", function () {
    this.closest(".result-card-main").classList.add("is-open");
  });

  collapse.addEventListener("hide.bs.collapse", function () {
    this.closest(".result-card-main").classList.remove("is-open");
  });
});
...
slider.noUiSlider.on("update", function (values) {
  document.getElementById("min").innerText = "€" + formatPrice(values[0]);
  document.getElementById("max").innerText = "€" + formatPrice(values[1]);
});

/// FILTER SELECT ALL ON CLICK CHECKBOX PAY NOW AND CAR SPECIFICATIONS

document
  .getElementById("payNowSelectAll")
  .addEventListener("change", function () {
    const items = document.querySelectorAll(".payNow-item");
    items.forEach((item) => {
...
document.querySelectorAll(".payNow-item").forEach((item) => {
  item.addEventListener("change", function () {
    const items = document.querySelectorAll(".payNow-item");
    const allChecked = [...items].every((i) => i.checked);
    document.getElementById("payNowSelectAll").checked = allChecked;
  });
});

///
document
  .getElementById("carSpecificationsSelectAll")
  .addEventListener("change", function () {
    const items = document.querySelectorAll(".carSpecifications-item");
    items.forEach((item) => {
      item.checked = this.checked;
    });
  });
...
// Optional: update "Select all" when individual checkboxes change
document.querySelectorAll(".carSpecifications-item").forEach((item) => {
  item.addEventListener("change", function () {
    const items = document.querySelectorAll(".carSpecifications-item");
    const allChecked = [...items].every((i) => i.checked);
    document.getElementById("carSpecificationsSelectAll").checked = allChecked;
  });
});
